...
export default function DashboardLayout({
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground font-body overflow-hidden">
      {/* Brightest Global Background Image */}
      <div 
        className="fixed inset-0 z-0 opacity-70 pointer-events-none bg-cover bg-center transition-opacity duration-1000" 
        style={{ backgroundImage: "url('https://picsum.photos/seed/ecopulse-bg-bright/1920/1080')" }}
        data-ai-hint="bright lush green nature"
      />
      
      {/* Light Overlay to pop the content and increase brightness */}
      <div className="fixed inset-0 z-0 bg-white/30 pointer-events-none" />

      <div className="relative z-10 flex h-full">
        <DashboardSidebar />
        <div className="flex-1 flex flex-col min-w-0 h-screen">
          <header className="h-16 border-b border-black/5 bg-white/70 backdrop-blur-md flex items-center justify-between px-8 sticky top-0 z-30">
